...
func countPaths1(n int, roads [][]int) int {
	...
	heap.Push(&hp, &IntItem{Value: n - 1, Priority: 0})
	...
	for hp.Len() > 0 {
		...
		visit[pop.Value] = true
		for y, d := range g[pop.Value] {
			if y == pop.Value {
				continue
			}
	...
	for i := 1; i < n; i++ {
		for y, d := range g[i] {
			// 可以这样计算的话是有个前提，也就是说在计算 i 的时候，这个 dp[y]是已经计算好的了，这样计算是不能满足这个要求的
			// 所以会出错
			if dis[i] == dis[y]+d {
				dp[i] += dp[y]
				dp[i] = dp[i] % base
...
func countPaths(n int, roads [][]int) int {
	...
	// 这里可以从0->n-1 递推，也可以从 n-1-->0这种方式，但是如果是从 0---> n-1，那么首先就得0入队，反之也是一样的
	dp[n-1] = 1
	hp := make(PriorityQueue, 0)
	heap.Push(&hp, &IntItem{Value: n - 1, Priority: 0})
	for hp.Len() > 0 {
