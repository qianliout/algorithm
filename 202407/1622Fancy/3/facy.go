...
func (t seg) do(O int, p pair) {
	...
	// 先乘后加
	if p.mul != 1 {
		// 在乘法更新时通常不需要使用 sz（区间大小），因为乘法操作是对节点当前存储的值进行的直接操作，而不是累加操作。
		// 对于线段树中的一个节点来说，它存储的值通常是其覆盖范围内的某些聚合信息（如总和、最小值、最大值等）。
		// 在这个特定的函数中，o.d.v 表示的是节点的某种聚合值。当执行乘法更新时，我们只需要将这个聚合值乘以给定的因子 p.mul 即可。
		// 这是因为乘法操作不会受到区间长度的影响——无论区间内有多少个元素，每个元素都应当被相同的因子所乘。
		// 例如，如果 o.d.v 存储的是一个区间的元素之和，那么当我们需要将区间内每个元素都乘以某个常数 k 时，
		// 我们只需将 o.d.v 乘以 k，而不需要知道具体的元素数量。
		// 因此，在乘法更新时不使用 sz 是合理的。而加法更新时使用 sz 是因为我们需要将区间内每个元素都加上一个常数值，
		// 所以需要知道区间的实际长度来正确地更新聚合值
		...
		// 组合操作：假设某个节点已经有一个待执行的加法操作 add 和一个待执行的乘法操作 mul。当新的乘法操作 p.mul 应用到这个节点时，
		// 我们需要确保之前的加法操作也能够正确地应用。
		// 具体来说：
